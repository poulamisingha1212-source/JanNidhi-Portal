...
- **Backend:** Python, FastAPI, SQLAlchemy ORM, PostgreSQL (with automatic SQLite development fallback).
- **Machine Learning & Risk Engine:** Scikit-Learn (Isolation Forest), XGBoost Regressor, custom rule-based deterministic scoring.
- **Frontend:** React 19, Tailwind CSS, Lucide Icons, Chart.js / Recharts.
- **Scheduling & Ingestion:** APScheduler running daily automated ingestion jobs.
- **Security:** Role-Based Access Control (RBAC), JWT / Header-based authentication, parameterized SQL queries, CORS protection.

### 2.2 System Architecture Diagram
```
[MoSPI Portal / Data Export]
            │
            ▼ (Daily Ingestion / APScheduler)
[FastAPI Ingestion Service] ───► [risk_engine.py] ◄─── [Pretrained ML Models (IForest, XGBoost)]
            │                           │
            ▼                           ▼
  [sync_log Table]             [PostgreSQL Database (works & reviews)]
                                        │
                                        ▼ (REST API Endpoints)
...
- **FR-2.2:** System shall maintain complete scoring consistency between `risk_engine.py`, PostgreSQL database, REST API, and the React frontend. Zero drift is permitted.
