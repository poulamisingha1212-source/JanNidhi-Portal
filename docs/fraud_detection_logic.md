...
To detect multi-dimensional outliers that sit just beneath individual rule thresholds, an Isolation Forest with 300 estimators is trained over:
- `cost_mad_score`
...
The raw decision score is converted to a portfolio percentile rank (`anomaly_percentile`) to ensure robustness against extreme outlier scaling.

### 3.3 Supervised Smoothing (XGBoost Regressor)
When trained on rule interactions, the XGBoost model outputs `model_risk_score`, which captures nonlinear relationships across administrative variables.
